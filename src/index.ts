...
export class DirectClient {
	...
	public startAgent: (agentId: string) => Promise<void> // Store startAgent functor
	...
	constructor() {
		...
		this.app.post('/:agentId/message', upload.single('file'), async (req: express.Request, res: express.Response) => {
			const agentId = req.params.agentId
		...
		this.app.post('/agents/:agentIdOrName/hyperfi/v1', async (req: express.Request, res: express.Response) => {
			// get runtime
			...
			function createHyperfiOutSchema(nearby: string[], availableEmotes: string[]) {
				const lookAtSchema =
					nearby.length > 1
						? z.union(nearby.map((item) => z.literal(item)) as [z.ZodLiteral<string>, z.ZodLiteral<string>, ...z.ZodLiteral<string>[]]).nullable()
						: nearby.length === 1
				...
				const emoteSchema =
					availableEmotes.length > 1
						? z.union(availableEmotes.map((item) => z.literal(item)) as [z.ZodLiteral<string>, z.ZodLiteral<string>, ...z.ZodLiteral<string>[]]).nullable()
						: availableEmotes.length === 1
		...
		this.app.get('/fine-tune/:assetId', async (req: express.Request, res: express.Response) => {
			...
			try {
				...
				const fileName = fileResponse.headers.get('content-disposition')?.split('filename=')[1]?.replace(/"/g, /* " */ '') || 'default_name.txt'
				...
				elizaLogger.log('File written successfully. Size:', stats.size, 'bytes')
		...
		this.app.post('/:agentId/speak', async (req, res) => {
			...
			// get elevenlabs settings from agent runtime
			const ELEVENLABS_VOICE_ID = runtime?.getSetting('ELEVENLABS_VOICE_ID')
			const ELEVENLABS_XI_API_KEY = runtime?.getSetting('ELEVENLABS_XI_API_KEY')
			const ELEVENLABS_MODEL_ID = runtime?.getSetting('ELEVENLABS_MODEL_ID')
			const ELEVENLABS_VOICE_STABILITY = runtime?.getSetting('ELEVENLABS_VOICE_STABILITY')
			const ELEVENLABS_VOICE_SIMILARITY_BOOST = runtime?.getSetting('ELEVENLABS_VOICE_SIMILARITY_BOOST')
			const ELEVENLABS_VOICE_STYLE = runtime?.getSetting('ELEVENLABS_VOICE_STYLE')
			const ELEVENLABS_VOICE_USE_SPEAKER_BOOST = runtime?.getSetting('ELEVENLABS_VOICE_USE_SPEAKER_BOOST')
			const ELEVENLABS_OPTIMIZE_STREAMING_LATENCY = runtime?.getSetting('ELEVENLABS_OPTIMIZE_STREAMING_LATENCY')
			const ELEVENLABS_OUTPUT_FORMAT = runtime?.getSetting('ELEVENLABS_OUTPUT_FORMAT')
			...
			try {
				...
				console.log('ElevenLabs Request:', {
					...
					modelId: ELEVENLABS_MODEL_ID || 'eleven_multilingual_v2',
					apiKey: ELEVENLABS_XI_API_KEY,
					text: textToSpeak,
					settings: {
						stability: Number.parseFloat(ELEVENLABS_VOICE_STABILITY || '0.5'),
						similarity_boost: Number.parseFloat(ELEVENLABS_VOICE_SIMILARITY_BOOST || '0.9'),
						style: Number.parseFloat(ELEVENLABS_VOICE_STYLE || '0.66'),
						use_speaker_boost: ELEVENLABS_VOICE_USE_SPEAKER_BOOST === 'true',
					},
				})
				...
				const elevenLabsApiUrl = `https://api.elevenlabs.io/v1/text-to-speech/${ELEVENLABS_VOICE_ID}`
				const apiKey = ELEVENLABS_XI_API_KEY
				...
				if (!speechResponse.ok) {
					...
					const responseBody = await speechResponse.json()
					console.log('ElevenLabs API error:', responseBody)
				...
				elizaLogger.error('Error processing message or generating speech:', error)
				res.status(500).json({
